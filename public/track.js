...
(function() {
  ...
  img.src = `${baseUrl}?site=${encodeURIComponent(siteId)}&path=${encodeURIComponent(currentPath)}&t=${Date.now()}`;
  img.style.cssText = 'position:absolute;left:-9999px;top:0;width:1px;height:1px;';
  img.alt = '';
  
  // Phase 2: JS実行検出（人間ブラウザ判定用）
  if (typeof fetch !== 'undefined') {
    fetch(`${baseUrl}/js-active?site=${encodeURIComponent(siteId)}&path=${encodeURIComponent(currentPath)}&t=${Date.now()}`, {
      method: 'GET',
      mode: 'no-cors',
      cache: 'no-cache'
    }).catch(() => {});
